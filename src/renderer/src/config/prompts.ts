...
export const LANG_DETECT_PROMPT = `Your task is to identify the language used in the user's input text and output the corresponding language from the predefined list {{list_lang}}. 

Special Rule 1: If Chinese characters are found in a mixed-language text, it is directly identified as \`zh-cn\`.
Special Rule 2: If uncertain, the default inference is the language other than \`zh-cn\` in the predefined list (usually \`en-us\`).

The user's input text will be enclosed within <text> and </text> XML tags. Don't output anything except the language code itself.
