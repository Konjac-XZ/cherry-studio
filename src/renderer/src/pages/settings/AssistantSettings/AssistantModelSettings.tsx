...
const AssistantModelSettings: FC<Props> = ({ assistant, updateAssistant, updateAssistantSettings }) => {
  ...
  const toolUseMode = useMemo<AssistantSettings['toolUseMode']>(
  ...
  const autoCopy = useMemo(
    () => assistant?.settings?.autoCopy ?? DEFAULT_ASSISTANT_SETTINGS.autoCopy,
    [assistant?.settings?.autoCopy]
  )
  const autoTranslate = useMemo(
    () => assistant?.settings?.autoTranslate ?? DEFAULT_ASSISTANT_SETTINGS.autoTranslate,
    [assistant?.settings?.autoTranslate]
  )
  const autoCleanupUserMessage = useMemo(
    () => assistant?.settings?.autoCleanupUserMessage ?? DEFAULT_ASSISTANT_SETTINGS.autoCleanupUserMessage,
    [assistant?.settings?.autoCleanupUserMessage]
  )
  ...
  const [maxToolCalls, setMaxToolCalls] = useState(
    assistant?.settings?.maxToolCalls ?? DEFAULT_ASSISTANT_SETTINGS.maxToolCalls
  )
